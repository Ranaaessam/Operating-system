...
struct Env* fos_scheduler_MLFQ() {
	...
	//	  Else, return NULL
	struct Env* D_queue;
	//check if curve is exist
	if (curenv != NULL) {
		//then put program in ready queue
		enqueue(&env_ready_queues[Q_index], curenv);
	}
	//loop from 0 until no. of queues entered by user
	for (uint32 i = 0; i < num_of_ready_queues; i++) {
		uint32 Q_size = queue_size(&env_ready_queues[i]);
		//check if size of given queue is not empty
		if (Q_size > 0) {
			D_queue = dequeue(&env_ready_queues[i]);
			kclock_set_quantum(quantums[i]);
			if (i < num_of_ready_queues - 1) {
				Q_index = i + 1;
			} else {
				Q_index = num_of_ready_queues - 1;
			}
			return D_queue;
		}
	}
