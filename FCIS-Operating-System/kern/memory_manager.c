...
void * create_page_table(uint32 *ptr_page_directory,
	...
	uint32 *new_PT = (void *) kmalloc(PAGE_SIZE);

	uint32 *ptrPT = NULL;
	//get_page_table(&ptr_page_directory , (const void *)new_PT , &ptrPT );
	struct Frame_Info *ptr_frame_info = get_frame_info(ptr_page_directory,
			new_PT, &ptrPT);

	uint32* new_PT_PhyAdd = (uint32*) kheap_physical_address((uint32) new_PT);
	uint32 indx = PDX(virtual_address);

	//Use CONSTRUCT_ENTRY() to build the table entry then link it to the given directory
	ptr_page_directory[indx] = CONSTRUCT_ENTRY((uint32 )new_PT_PhyAdd,
	...
	memset((uint32*) new_PT, 0, PAGE_SIZE);
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	int no_of_pages = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	for (uint32 x = virtual_address; x <= size; x += PAGE_SIZE)
	{
		pf_remove_env_page(e, x);
		for (uint32 y = 0; y < e->page_WS_max_size; y++) {
			uint32 viradd = env_page_ws_get_virtual_address(e, y);
			if (viradd <= size && viradd >= virtual_address) {
				unmap_frame(e->env_page_directory, (void*) viradd);
				env_page_ws_clear_entry(e, y);
				break;
			}
		}
		uint32* PageTablePtr;
		get_page_table(e->env_page_directory, (void*) x, &PageTablePtr);
		bool flag = 0;
		if (PageTablePtr != NULL) {
			for (uint32 k = 0; k < 1024; k++) {
				if ((PageTablePtr[k] & PERM_PRESENT) == 1) {
					flag = 1;
					break;
				}
			}
			if (flag == 0) {
				pd_clear_page_dir_entry(e, x);
				kfree((void*) PageTablePtr);
			}
			flag = 0;
		}
	}
